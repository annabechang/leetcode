...
class Solution:
    def romanToInt(self, s: str) -> int:
        #use dictionary, so don't need to write every ifelse for every symbol 
        
        numeral_map = {"I": 1, "V": 5, "X": 10, "L":50, "C":100, "D":500, "M":1000}
        result = 0
        
        for i in range(len(s)):
            #is the current symbol bigger than the previous one?
            #if yes, current - previous
            #if not, current +
            if i > 0 and numeral_map[s[i]] > numeral_map[s[i-1]]:
                result += numeral_map[s[i]] - 2 * numeral_map[s[i-1]]
            # *2 because we have to minus the previous added one
            # without *2 will be:
            #result = M -> M+C -> M+C+M-C = 2M which is not correct 
